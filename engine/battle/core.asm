...
.playerPriorityMoveLoop
	...
	inc a       ; if at the end of list: -1 + 1 = 0xFF + 0x01 = 0
...
.enemyPriorityMoveLoop
	ld a, [hli] ; load the move ID from priority list and
	            ; increment address to the priority value address
	cp d        ; compare with the move being used
...
DrawEnemyHUDAndHPBar:
	...
	; start of code to add the caught symbol by RainbowMetalPigeon
	push hl
	ld a, [wEnemyMonSpecies2]
	ld [wd11e], a
	...
	ld [hl], $72 ; replace this with Poke Ball icon or character
.notOwned
	pop hl
	; end of new code
	ld de, wEnemyMonNick
...
.MonHurtItselfOrFullyParalysed
	ld hl, wPlayerBattleStatus1
	ld a, [hl]
	; clear bide, thrashing, charging up, trapping moves such as warp (already cleared for confusion damage), and invulnerable moves
...
.loop
	...
	inc a                        ; move on the next move, FF terminates loop
...
.HighCritical
	...
	ld b, $ff                    ; cap at 255
.noCarry
	sla b                        ; *4 for high critical hit move
...
.monHurtItselfOrFullyParalysed
	ld hl, wEnemyBattleStatus1
	ld a, [hl]
	; clear bide, thrashing about, charging up, multi-turn moves such as warp (already cleared for confusion damage), and invulnerable moves
	and ~((1 << STORING_ENERGY) | (1 << THRASHING_ABOUT) | (1 << CHARGING_UP) | (1 << USING_TRAPPING_MOVE) | (1 << INVULNERABLE))
	ld [hl], a
	ld a, [wEnemyMoveEffect]
	cp FLY_EFFECT
